...
def render_normal(viewpoint_cam, depth, bg_color, alpha):
    ...
    normal_ref = normal_from_depth_image(
        depth, intrinsic_matrix.to(depth.device), extrinsic_matrix.to(depth.device)
    )
    background = bg_color[None, None, ...]
...
def render(
    ...
    if colors_precomp is None:
        if pipe.brdf:
            ...
            if pipe.brdf_mode == "envmap":
                ...
                view_pos = viewpoint_camera.camera_center.repeat(
                    pc.get_opacity.shape[0], 1
                )  # (N, 3)

                diffuse = pc.get_diffuse  # (N, 3)
                normal, delta_normal = pc.get_normal(
                    dir_pp_normalized=dir_pp_normalized, return_delta=True
                )  # (N, 3)
    ...
    if not speed:
        ...
        p_hom = torch.cat(
            [pc.get_xyz, torch.ones_like(pc.get_xyz[..., :1])], -1
        ).unsqueeze(-1)
        p_view = torch.matmul(
            viewpoint_camera.world_view_transform.transpose(0, 1), p_hom
        )
        p_view = p_view[..., :3, :]
        ...
        for k in ["normal", "normal_axis"] if debug else ["normal"]:
            if k in out_extras.keys():
                out_extras[k] = (out_extras[k] - 0.5) * 2.0  # range (0, 1) -> (-1, 1)
        ...
        if debug:
            out_extras["normal_ref_cam"] = rendered_world2cam(
                viewpoint_camera,
                out_extras["normal_ref"],
                out_extras["alpha"][0],
                bg_color,
            )
            out_extras["normal_axis_cam"] = rendered_world2cam(
                viewpoint_camera,
                out_extras["normal_axis"],
                out_extras["alpha"][0],
                bg_color,
            )
        ...
        if pipe.brdf:
            ...
            if debug:
                out_extras["normal_cam"] = rendered_world2cam(
                    viewpoint_camera,
                    out_extras["normal"],
                    out_extras["alpha"][0],
                    bg_color,
