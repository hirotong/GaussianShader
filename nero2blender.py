...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--path",
        type=str,
        default="/remote-home/tujd/NeRF/gaussian-splatting-surfel/data/GlossySynthetic",
        help="path to the GlossyBlender dataset",
    )
    ...
    cams = [
        read_pickle(f"{root}/{k}-camera.pkl") for k in range(img_num)
    ]  # pose(3,4)  K(3,3)
    ...
    for split in ["train", "test"]:
        ...
        for image, cam in zip(split_imgs, split_cams):
            ...
            frames.append(
                {
                    "file_path": os.path.join("rgb", os.path.basename(image)).replace(
                        ".png", ""
                    ),
