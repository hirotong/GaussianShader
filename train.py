...
from utils.image_utils import psnr, srgb2linear, linear2srgb
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations):
    ...
    gaussians = GaussianModel(
        dataset.sh_degree, dataset.brdf_dim, dataset.brdf_mode, dataset.brdf_envmap_res
    )
    ...
    for iteration in range(1, opt.iterations + 1):
        ...
        if pipe.brdf:
            gaussians.set_requires_grad(
                "normal", state=iteration >= opt.normal_reg_from_iter
            )
            gaussians.set_requires_grad(
                "normal2", state=iteration >= opt.normal_reg_from_iter
            )
        ...
        if pipe.brdf and iteration > opt.normal_reg_from_iter:
            ...
            if (
                "delta_normal_norm" not in render_pkg.keys()
                and opt.lambda_delta_reg > 0
            ):
                assert ()
    ...
    if tb_writer:
        ...
        for k in losses_extra.keys():
            tb_writer.add_scalar(
                f"train_loss_patches/{k}_loss", losses_extra[k].item(), iteration
            )
                    ...
                    if tb_writer and (idx < 5):
                        ...
                        for k in render_pkg.keys():
                            if (
                                render_pkg[k].dim() < 3
                                or k == "render"
                                or k == "delta_normal_norm"
                            ):
                                continue
                            if k == "depth":
                                image_k = apply_depth_colormap(
                                    -render_pkg[k][0][..., None]
                                )
                                image_k = image_k.permute(2, 0, 1)
                            elif k == "alpha":
                                image_k = apply_depth_colormap(
                                    render_pkg[k][0][..., None], min=0.0, max=1.0
                                )
                                image_k = image_k.permute(2, 0, 1)
                            else:
                                if "normal" in k:
                            ...
                            tb_writer.add_images(
                                config["name"]
                                + "_view_{}/{}".format(viewpoint.image_name, k),
                                image_k[None],
                                global_step=iteration,
                            )
                        ...
                        if renderArgs[0].brdf:
                            lighting = render_lighting(
                                scene.gaussians, resolution=(512, 1024)
                            )
                            if tb_writer:
                                tb_writer.add_images(
